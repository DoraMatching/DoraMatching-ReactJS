...
function Home({home, classes}) {
    ...
    return (
        <div style={{width: '100%'}} className={`${styles.container} ${styles.homeGrid}`}>
            <div style={{width: '100%', paddingLeft: '20px', gridColumn: '1/3'}}>{renderComponents()}</div>
            <div style={{width: '100%', paddingRight: '20px', marginTop: '20px'}}>
                {user && <ScheduleTrainer user={user}/>}
                <div style={{marginTop: '20px'}}>
                    {classes.slice(0, 5).map((classe, id) => {
...
Home.getInitialProps = async () => {
    const [home, classes] = await Promise.all([Client('home'), Client(`classes`)]);
    return {
